...
// Helper function to rollback images only
async function rollbackImages(rollbackData) {
    try {
        // Delete newly uploaded images from Cloudinary
        if (rollbackData.uploadedImages.length > 0) {
            const deletePromises = rollbackData.uploadedImages.map((image) => cloudinary.v2.uploader.destroy(image.public_id))
            await Promise.all(deletePromises)
        }

        // Note: We cannot restore the originally deleted images from Cloudinary
        // as they are permanently deleted. Consider keeping a backup strategy
        // or implementing a soft delete mechanism for critical applications
